...
The results were plotted. First plot represented contribution of principal components to the variance of the data (Figure 8.1). Second plot is the scatter plot individuals (our data) allowed us to visualize the groupings among accessions (Figure 8.2). The third plot is the loadings correlation circle. It gives additional idea about how the variables contribute to data projection on the principal components. And the last plot was used to conclude the choice of main variables, that contributes to salt tolerance (Figure 8.3). Chosen variables were later used in MFV ranking .
...
The dendrogram was established based on MFV mean (6 highest). And 3 groups were formed. 
...
knitr::kable(STI, caption = "Table 4.1. Summary table for salt tolerance index", align="c", digits = round(2))
```
...
#Creating a table with the results and a caption for it
knitr::kable(MFV, caption = "Table 5.1. MFV table based on STI", align="c", digits = round(2))
```
...
#making a score rplot of PC1 and PC2
...
The only accession, whose tolerance might be linked to location is Ess-1, that is closest to the shore.
