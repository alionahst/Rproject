...
In order to evaluate degree of salt tolerance (define the susceptible and resistant accessions), first we needed to convert all morpho-physiological parameters (control and treatment) for each accession to the salt-tolerance index (STI) - ratio of treated plant values to control.

Here we deviated from the 

we needed to rank the 
We created another table with data converted into salt tolerance index (STI): .
...
### 6. Salt-tolerance evaluation: ranking and dendogram 
In order to evaluate salt tolerance, we made accessions groups that behave similarly under salt stress. This was based on the MFV mean of the 8 highest ranked physio-morphological parameters from PCA analysis. 

The ranking was done by ordering MFV mean from highest MFV mean to the smallest one. Then the groups were adressed according to the same logic as the paper (with higher MFV mean corresponding to higher salt tolerance): 4% accessions as highly tolerant, 16% as tolerant, 60% as moderately tolerant, 16% as sensitive and finally 4% as highly sensitive.

The dendogram was established based on MFV mean. And 4 groups were formed as in the paper work. 

### 7.Accessions map and location correlation ?
...
```
#### (9) Accessions map and location correlation
...
knitr::kable(cor_1_result, caption = "Table 9.1. Correlation of mean MFV and elevation", align="c")
```
