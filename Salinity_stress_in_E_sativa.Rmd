...
ggplot(map.acc, aes(x = Longitude, 
  ...
  labs(colour = "Categories according to salt tolerance ranking") + #legend title
  geom_point() + #making points of accessions visible
  geom_text(aes(label = code), #creating labels for accessions
            nudge_x = -0.2,    #shifting the position of labels
            size = 3,
            check_overlap = T) + #don't allow to use overlapping text
  borders(database = "world", regions = "Pakistan") + #adding Pakistan layer
  ggtitle("Accessions location in Pakistan") + #adding table title
  theme_minimal()
